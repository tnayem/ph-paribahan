...
let totalDiscountAmount = document.getElementById('totalDiscount')
let bookingSeats = []
    ...
    seat.addEventListener("click", function (e) {
        ...
        if (e.target.innerText === bookingSeats[0] || e.target.innerText === bookingSeats[1] || e.target.innerText === bookingSeats[2] || e.target.innerText === bookingSeats[3]) {
            ...
            if (quantityBukingSeats > 4) {
                ...
                if(quantityBukingSeats=== 4){
                    const applyCopon = document.getElementById('apply-copun')
                    applyCopon.disabled = false
                    couponBtn.addEventListener('click',function(){
                      ...
                      if(couponCodeValue === "NEW15"){
                        ...
                        totalDiscountAmount.innerText = totalDiscount;
                        grandTotalPrice.innerText = totalSitPrice - totalDiscount;
                        applyCopon.disabled = true
                        couponCode.value = " "
                      }
...
number.addEventListener('input',function(e){
    if(e.target.value.length>7 && bookingSeats.length > 0){
